            ...
            for i in range(1, 5):
                ...
                for sent in preds[i]:
                    ...
                    if sent in pred['gold_sent']:
                        if pred_subphrase is not None:
                            problem_count[i][2] += 1
                        if pred_superphrase is not None:
                            problem_count[i][3] += 1
                    else:
                        if pred_subphrase is not None:
                            problem_count[i][0] += 1
                ...
                for gd in gds[i]:
                    ...
                    if gd in pred['pred_sents']:
                        if gd_subphrase is not None:
                            problem_count[4 if len(gd_subphrase) > 4 else len(gd_subphrase)][3] += 1
                        if gd_superphrase in pred['pred_sents']:
                            problem_count[4 if len(gd_superphrase) > 4 else len(gd_superphrase)][2] += 1
                            print(gd_superphrase)
                    else:
                        if gd_subphrase is not None:
                            problem_count[4 if len(gd_subphrase) > 4 else len(gd_subphrase)][1] += 1
                        if gd_superphrase is not None:
                            problem_count[4 if len(gd_superphrase) > 4 else len(gd_superphrase)][0] += 1
                            print(gd_superphrase)
